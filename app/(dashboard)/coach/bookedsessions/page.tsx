...
import { getBookedSessions } from '@/services/api'
...
export default function BookedSessionsPage() {
  ...
  useEffect(() => {
    fetch(`/api/coach/profile?userId=${COACH_USER_ID}`)
      .then(r => r.json())
      .then(profile => getBookedSessions(profile.id))
      .then(setSessions)
      .catch(console.error)
          ...
          <div className="flex items-center gap-2">
            ...
            {(['all', 'confirmed', 'pending', 'cancelled'] as const).map((s) => (
              <button key={s} onClick={() => setFilterStatus(s)}
                className={`px-3 py-1 rounded-full text-xs font-medium capitalize transition-colors ${filterStatus === s ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-500 hover:bg-gray-50'
                  }`}
              >
                {s}
              </button>
