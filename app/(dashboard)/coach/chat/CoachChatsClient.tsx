  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 bg-blue-50 border-t border-blue-100">
      <div className="w-10 h-10 rounded-xl bg-white border border-blue-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
        {pending.file.type.startsWith('image/') && pending.previewUrl
  ...
  const fetchConversations = useCallback(async (userId: string) => {
    ...
    if (!error && data) {
      const shaped = data.map((c: any) => ({
        ...c, athlete: Array.isArray(c.athlete) ? c.athlete[0] ?? null : c.athlete,
      })) as Conversation[]
                ...
                <div className="relative flex items-center gap-3">
                  ...
                  <button onClick={sendMessage}
                    ...
                    className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors flex-shrink-0 ${
                      (newMessage.trim() || pendingFile) ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400 cursor-default'
                    }`}>
