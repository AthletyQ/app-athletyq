...
export default function ConsultantChatPageClient() {
  ...
  const fetchConversations = useCallback(async (userId: string) => {
    ...
    if (!error && data) {
      ...
      if (targetId) {
        const target = shaped.find((c) => c.id === targetId)
        if (target) {
            ...
            filtered.map((conv) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50 border-b border-gray-50 ${isActive ? 'bg-indigo-50 border-l-2 border-l-indigo-500' : ''}`}
                >
                  <div
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-[70%] ${isMine ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
                        <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMine ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-gray-100 text-gray-800 rounded-bl-sm'}`}>
                          {msg.content}
                        </div>
