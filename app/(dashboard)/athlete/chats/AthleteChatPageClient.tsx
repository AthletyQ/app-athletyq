...
export default function AthleteChatPageClient() {
  ...
  return (
    <>
      {deleteTarget && (
        <DeleteModal name={fullName(deleteTarget.contact)} onConfirm={handleDeleteConversation}
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingConvs ? (
              ...
              <div className="flex flex-col items-center justify-center h-48 gap-2 px-6 text-center">
                ...
                <p className="text-xs text-gray-300">Book a session to start chatting</p>
              </div>
            ) : (
              filtered.map((conv) => {
                ...
                return (
                  <div key={conv.id}
                    ...
                    <div className="w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-semibold flex-shrink-0"
                      style={{ backgroundColor: avatarColor(conv.contact_id) }}>{initials(conv.contact)}</div>
                    <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200 flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-semibold flex-shrink-0"
                    style={{ backgroundColor: avatarColor(activeConv.contact_id) }}>{initials(activeConv.contact)}</div>
                  <div>
                    <p className="text-sm font-bold text-gray-900">{fullName(activeConv.contact)}</p>
