...
//   // Never show action buttons on your own card OR to non-athletes
//   const showActions = isAthlete && currentUserId !== consultant.id;
...
//       if (existing) {
//         router.push(`/athlete/chats?contactId=${consultant.id}`);
//         return;
//       }
...
//         {/* Actions — only for athletes, never on own card */}
//         {showActions && (
//           <div className="flex gap-2 mt-auto">
//             <button
//               onClick={handleMessage}
...
export default function ConsultantCard({ consultant }: { consultant: Consultant }) {
  ...
  const [isBooking,     setIsBooking]     = useState(false);
  const [messaging,     setMessaging]     = useState(false);
  const [isAthlete,     setIsAthlete]     = useState(true);   // optimistic: show until confirmed otherwise
  const [roleLoaded,    setRoleLoaded]    = useState(false);  // true once profile fetch resolves
  const [currentUserId, setCurrentUserId] = useState<string | null>(null);
  ...
  // Show buttons optimistically; hide only after role confirmed as non-athlete or own card
  const showActions = (!roleLoaded || isAthlete) && currentUserId !== consultant.id;
